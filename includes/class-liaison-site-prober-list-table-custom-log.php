...
class LIAISIPR_List_Table_Custom_Log extends WP_List_Table {
	...
	protected function get_session_generate_url() {
		return wp_nonce_url(
			...
			'wpsp_session_generate_custom_log',
			'wpsp_nonce_custom_log'
		);
	}
	...
	protected function render_session_generate_button() {
		printf(
			...
			esc_html__('Custom Session Generate', 'liaison-site-prober')
		);
	}
	...
	protected function render_export_button() {
		printf(
			...
			esc_html__('Export CSV (Custom Log)', 'liaison-site-prober')
		);
	}

	protected function get_cached_list($cache_key, $callback) {
		$group = 'liaison-site-prober';
		$cached = wp_cache_get($cache_key, $group);
		if ($cached !== false) {
			return $cached;
		}
		$result = call_user_func($callback);
		wp_cache_set($cache_key, $result, $group, 5 * MINUTE_IN_SECONDS);
		return $result;
	}
	...
	public function load_filter_options() {
		...
		return $this->get_cached_list('custom_log_filter_options', function() use ($wpdb, $table) {
			return [
				'plugins' => $wpdb->get_col(
					"SELECT DISTINCT plugin_name FROM `{$table}` ORDER BY plugin_name DESC LIMIT 200"
				),
				'severity' => $wpdb->get_col(
					"SELECT DISTINCT severity FROM `{$table}` ORDER BY severity DESC LIMIT 200"
				),
			];
		});
	}
	...
	protected function render_severity_filter($list, $selected) {
		echo '<select name="severityshow" id="severityshow">';
		echo '<option value="">' . esc_html__('All Severity', 'liaison-site-prober') . '</option>';
	...
	public function log_plugin_select( $plugin_select ) {
		...
		error_log( sprintf('log_plugin_select : %s', $plugin_select) );		
		if ( '' === $plugin_select ) {
			return false;
		}
		$where = $wpdb->prepare( 
	...
	public function build_query_args() {
		...
		if ( ! empty( $_REQUEST['log-select'] ) ) {
			$where .= $wpdb->prepare(
				' AND message = %s',
				sanitize_text_field( wp_unslash( $_REQUEST['log-select'] ) )
			);
		}
